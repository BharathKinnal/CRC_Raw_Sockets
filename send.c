...
int main (int argc, char **argv)
{
  ...
  sprintf (payload, "Geetha Ma'am is a very good teacher. She taught us raw sockets.");
...
int j,temp,n;
	...
	bol m[IP_MAXPACKET],p[IP_MAXPACKET];
	for(i=0;i<=framelength;i++)
	{
		uint8_t nonsense=*maamu;
		for(j=0;j<8;j++){
			p[i].b[j]=m[i].b[j]=nonsense%2;
			nonsense/=2;
		}
	...
	for(j=0;j<n;j++)
	{
		...
		{
			temp=m[j].b[i];
			p[j].b[i]=m[j].b[i]=m[j].b[7-i];
			p[j].b[7-i]=m[j].b[7-i]=temp;
		}
	}
	...
	for(i=8;i<8*(n+1);i++)
		if(m[i/8-1].b[i%8]==1)
			for(j=0;j<9;j++){
				if()
				m[i/8-1].b[i%8+j]=m[i/8+j].b[i%8+j]^g[j];
			}
